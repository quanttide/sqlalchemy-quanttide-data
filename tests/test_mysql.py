...
print('\nsql_client.pymysql:')
db = sql_client.pymysql.SqlClient(raise_error=True, **env.mysql)
...
db = sql_client.sqlalchemy.SqlClient(dialect='mysql', raise_error=True, **env.mysql)
# print(db.query('insert into t values (1,2)', fetchall=False))
...
db.try_connect()
print(db.query('select * from t', origin_result=True))
db = sql_client.sqlalchemy.SqlClient(dialect='mysql', autocommit=False, raise_error=True, **env.mysql)
print(db.query('select * from t', origin_result=True))
...
db = sql_client.pymysql.SqlClient(raise_error=True, **env.mysql_punc)
...
db = sql_client.sqlalchemy.SqlClient(dialect='mysql', raise_error=True, **env.mysql_punc)
# print(db.query('insert into {} values (1,2)'.format(table), fetchall=False))
...
db.try_connect()
print(db.query('select * from {}'.format(table), origin_result=True))
db = sql_client.sqlalchemy.SqlClient(dialect='mysql', autocommit=False, raise_error=True, **env.mysql_punc)
print(db.query('select * from {}'.format(table), origin_result=True))
